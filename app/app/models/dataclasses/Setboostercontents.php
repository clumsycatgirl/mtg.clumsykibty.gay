<?php

namespace App\Models\DataClasses;

/**
 * Class Setboostercontents
 *
 * Represents a Setboostercontents in the system.
 */
class Setboostercontents {
    private int|null $id;
    private int|null $booster_index;
    private string|null $booster_name;
    private string|null $set_code;
    private string|null $sheet_name;
    private int|null $sheet_picks;

    /**
     * Constructor.
     *
     * @param array $array The row from the database.
     */
    public function __construct(array $array) {
        $this->id = $array['id'] ?? null;
        $this->booster_index = $array['boosterIndex'] ?? null;
        $this->booster_name = $array['boosterName'] ?? null;
        $this->set_code = $array['setCode'] ?? null;
        $this->sheet_name = $array['sheetName'] ?? null;
        $this->sheet_picks = $array['sheetPicks'] ?? null;
    }

    /**
     * Get the id.
     *
     * @return int|null
     */
    public function id(): int|null {
        return $this->id;
    }

    /**
     * Get the booster_index.
     *
     * @return int|null
     */
    public function booster_index(): int|null {
        return $this->booster_index;
    }

    /**
     * Get the booster_name.
     *
     * @return string|null
     */
    public function booster_name(): string|null {
        return $this->booster_name;
    }

    /**
     * Get the set_code.
     *
     * @return string|null
     */
    public function set_code(): string|null {
        return $this->set_code;
    }

    /**
     * Get the sheet_name.
     *
     * @return string|null
     */
    public function sheet_name(): string|null {
        return $this->sheet_name;
    }

    /**
     * Get the sheet_picks.
     *
     * @return int|null
     */
    public function sheet_picks(): int|null {
        return $this->sheet_picks;
    }

}
